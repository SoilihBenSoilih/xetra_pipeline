...
class XetraETL():
    ...
    def extract(self):
        ...
        files = [
            key for date in self.extract_date_list for key in self.s3_bucket_src.list_files_in_prefix(date)
        ]
        if not files:
    ...
    def transform_report1(self, data_frame: pd.DataFrame):
        ...
        data_frame[self.trg_args.trg_col_ch_prev_clos] = (
            data_frame[self.trg_args.trg_col_op_price] - data_frame[self.trg_args.trg_col_ch_prev_clos]
